...
1. **下载订阅配置文件：**
   ```bash
   curl -L -o config.yaml "https://api-huacloud.net/sub?target=clash&insert=true&emoji=true&udp=true&clash.doh=true&new_name=true&filename=Flower_SS.yaml&url=https%3A%2F%2Fapi.xmancdn.net%2Fosubscribe.php%3Ftoken2%3Dq963pegy-e6f32f41ceyr4fi43r%26sip002%3D1"
   ```
2. **第一次手动测试 (前台运行)：**
   ```bash
   ./mihomo -d .
